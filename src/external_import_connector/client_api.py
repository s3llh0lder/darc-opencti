...
class ConnectorClient:
    ...
    def generate_stix_from_text(self, text: str) -> dict:
        ...
        try:
            ...
            self.helper.connector_logger.info(f"Successfully received STIX object {stix_output}")
        ...
        try:
            ...
            for obj in parsed.get("objects", []):
                if not isinstance(obj, dict):
                    self.helper.connector_logger.warning("Skipping non-dictionary object")
                    continue
                ...
                try:
                    ...
                    if uuid_str != canonical_uuid:
                        ...
                        self.helper.connector_logger.info(f"Reformatted UUID: {obj_id} -> {obj['id']}")

                except (ValueError, AttributeError, IndexError) as e:
                    # Generate new UUIDv4 and preserve relationships
                    original_id = obj.get("id", "missing-id")
                    new_uuid = uuid.uuid4()
                    prefix = obj.get("type", "unknown").lower().replace("_", "-").replace(" ", "-")
